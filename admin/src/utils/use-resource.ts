...
import { Category, ExternalUser, Image, Scenario, ScenarioIntroText } from '@data-contracts/backend/data-contracts';
...
export const useResource = (resource: ResourceName) => {
  ...
  const refresh = useCallback(() => {
    if (getMany) {
      ...
      handleGetMany<Image | Scenario | Category | ScenarioIntroText | ExternalUser>(() => getMany())
        .then((res) => {
          if (res) {
