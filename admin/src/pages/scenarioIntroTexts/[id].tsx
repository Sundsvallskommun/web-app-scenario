import { EditScenarioIntroText } from '@components/edit-scenario-intro-text/edit-scenario-intro-text.component';
import { EditorToolbar } from '@components/editor-toolbar/editor-toolbar';
import LoaderFullScreen from '@components/loader/loader-fullscreen';
import { defaultInformationFields } from '@config/defaults';
import resources from '@config/resources';
import { CreateScenarioIntroTextDto, UpdateScenarioIntroTextDto } from '@data-contracts/backend/data-contracts';
...
export const ScenarioIntroTextPage: React.FC = () => {
  ...
  type CreateType = CreateScenarioIntroTextDto;
  type UpdateType = UpdateScenarioIntroTextDto;
  ...
  const isDuplicateSortOrderError = (error: unknown) => {
    const message = (error as AxiosError<{ message?: string }>)?.response?.data?.message ?? '';
    return typeof message === 'string' && message.includes('sortOrder');
  };
  ...
  const onSubmit = async (data: DataType) => {
    ...
    try {
      switch (isNew) {
        case true:
          if (create) {
            const result = await create(data as CreateType);
            if (result) {
              message({ message: capitalize(t('crud:create.success', { resource: t(`${resource}:name_one`) })), status: 'success' });
        ...
        case false:
          if (id) {
            const result = await (update?.(id, data) as ResourceResponse<Partial<FieldValues>>);
            message({ message: capitalize(t('crud:update.success', { resource: t(`${resource}:name_one`) })), status: 'success' });
            reset(result.data.data);
            refresh();
          }
          break;
      ...
      if (isDuplicateSortOrderError(error)) {
        setError('sortOrder', {
          type: 'server',
          message: t(`${resource}:validation.sortOrderUnique`),
        });
        return;
      }

      message({
        message: capitalize(t(isNew ? 'crud:create.error' : 'crud:update.error', { resource: t(`${resource}:name_one`) })),
        status: 'error',
      });
    }
  };
...
export default ScenarioIntroTextPage;
