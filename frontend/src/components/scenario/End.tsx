...
export const End: React.FC<EndProps> = ({ history, transitionDuration = 1000, onRestart }) => {
  ...
  const entry = history.filter((entry: ChatHistoryEntry) => entry.origin === 'assistant').at(-1);
    ...
    <>
      ...
      >
        <header className="flex flex-col items-center text-center py-80">
          <h1 className="text-display-1-sm md:text-display-1-md xl:text-display-1-lg">{t('common:app_name')}</h1>
          <p className="text-display-3-sm md:text-display-3-md xl:text-display-3-lg">{t('common:scenario_over')}</p>
        </header>
        <div
        ...
        >
          {!entry || !entry.done ?
            <Spinner />
          : <AIGameFeed history={[entry]} className="w-full max-w-[1000px] mx-auto" />}
        </div>
